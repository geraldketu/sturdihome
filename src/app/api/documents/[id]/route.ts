...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (user.role !== "HOMEOWNER" && user.role !== "ADMIN") return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  ...
  try {
    const buffer = await readUpload(document.userId, document.fileName);
