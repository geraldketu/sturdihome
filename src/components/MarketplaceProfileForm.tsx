...
export default function MarketplaceProfileForm({ initial, financing, adminVendorId, adminOwnerId }: { initial: ListingFields; financing: boolean; adminVendorId?: string; adminOwnerId?: string }) {
  ...
  return <form action={action} className="space-y-5">
    {adminVendorId && <input type="hidden" name="adminVendorId" value={adminVendorId} />}
    {adminOwnerId && <input type="hidden" name="adminOwnerId" value={adminOwnerId} />}
    ...
    {(adminVendorId || adminOwnerId) && <div className="grid gap-3 sm:grid-cols-2"><label className="flex items-center gap-2 text-sm"><input type="checkbox" name="featured" defaultChecked={false} /> Feature this listing</label><label className="text-sm">Featured order<input className={field} name="sortOrder" type="number" defaultValue="0" /></label></div>}
