...
const GREETINGS: Record<Character, string> = {
  sturdiGirl: "Hi, I am Sturdi Girl. I am your guide through SturdiHome, and I can help you find services, understand next steps, and explore financing without confusing us with a lender.",
  brixy: "Well hello there. I am Brixy, Sturdi Girl's dependable sidekick. I will keep an eye on the details while she keeps us moving. No need to bark up the wrong page.",
};
const QUICK_LINKS = [
...
export default function ChatWidget({ authenticated = false, initialStatus = null, paymentsEnabled = false }: { authenticated?: boolean; initialStatus?: Status | null; paymentsEnabled?: boolean }) {
  ...
  const [character, setCharacter] = useState<Character>("sturdiGirl");
  ...
  const { messages, sendMessage, status: chatStatus, error } = useChat<UIMessage>({ transport: new DefaultChatTransport({ api: "/api/chat" }), messages: [{ id: "greeting", role: "assistant", parts: [{ type: "text", text: GREETINGS.sturdiGirl }] }] });
  ...
  return <div className="character-widget fixed bottom-4 right-4 z-50 flex flex-col items-end gap-3 sm:bottom-6 sm:right-6">
    {open && <div className={`character-panel character-panel-${panelMode} flex flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-xl`}>
      <div className="flex items-center justify-between bg-brand-navy px-4 py-3">
          <div className="flex items-center gap-2">{activeCharacter && <CharacterStage character={character} className="character-stage-static h-10 w-10 overflow-hidden rounded-full border border-brand-gold" />}<div><p className="text-sm font-semibold text-white">{activeCharacter ? character === "sturdiGirl" ? "Sturdi Girl" : "Brixy" : "Choose your guide"}</p><p className="text-xs text-brand-gold-pale">{activeCharacter ? character === "sturdiGirl" ? "Your guide" : "Guide companion" : "Start inside the AI experience"}</p><UsageStatus status={status} /></div></div>
      ...
      <div className="flex border-b border-gray-200 bg-white p-2"><button type="button" onClick={() => interactWith("sturdiGirl")} className={`flex flex-1 items-center justify-center gap-1 rounded-md px-2 py-1.5 text-xs font-semibold ${character === "sturdiGirl" ? "bg-brand-navy text-white" : "text-brand-navy"}`}><CharacterStage character="sturdiGirl" motion={activeCharacter === "sturdiGirl" ? "speaking" : "idle"} className="h-6 w-6 overflow-hidden rounded-full" />Sturdi Girl</button><button type="button" onClick={() => interactWith("brixy")} className={`flex flex-1 items-center justify-center gap-1 rounded-md px-2 py-1.5 text-xs font-semibold ${character === "brixy" ? "bg-brand-navy text-white" : "text-brand-navy"}`}><CharacterStage character="brixy" motion={activeCharacter === "brixy" ? "speaking" : "idle"} className="h-6 w-6 overflow-hidden rounded-full" />Brixy</button></div>
      <div className="grid grid-cols-2 gap-2 px-3 pt-3"><div className="space-y-1"><button type="button" onClick={() => interactWith("sturdiGirl")} className="w-full rounded-md bg-brand-navy px-2 py-2 text-xs font-semibold text-white">Interact with Sturdi Girl</button>{activeCharacter === "sturdiGirl" && <button type="button" onClick={() => stopInteraction("sturdiGirl")} className="w-full rounded-md border border-brand-navy/20 px-2 py-1.5 text-xs font-semibold text-brand-navy">Stop Sturdi Girl</button>}</div><div className="space-y-1"><button type="button" onClick={() => interactWith("brixy")} className="w-full rounded-md bg-brand-navy px-2 py-2 text-xs font-semibold text-white">Interact with Brixy</button>{activeCharacter === "brixy" && <button type="button" onClick={() => stopInteraction("brixy")} className="w-full rounded-md border border-brand-navy/20 px-2 py-1.5 text-xs font-semibold text-brand-navy">Stop Brixy</button>}</div></div>
      {activeCharacter && <div className="mx-3 mt-3 rounded-md border border-brand-gold/40 bg-brand-gold-pale/15 p-2"><p className="mb-2 text-xs font-semibold text-brand-navy">{activeCharacter === "sturdiGirl" ? "Sturdi Girl can guide you to:" : "Brixy can help you find:"}</p><div className="grid grid-cols-2 gap-1">{GUIDE_CHOICES.map(([label, href]) => <Link key={href} href={href} className="rounded border border-brand-navy/10 bg-white px-2 py-1.5 text-[11px] text-brand-navy hover:bg-brand-gold-pale/30">{label}</Link>)}</div></div>}
      {intro && <div className="m-3 rounded-lg border border-brand-gold/40 bg-brand-gold-pale/20 p-3 text-xs leading-5 text-brand-navy"><div className="mb-2 flex gap-2"><CharacterStage character="sturdiGirl" motion="listening" className="h-12 w-12 overflow-hidden rounded-full" /><CharacterStage character="brixy" motion="listening" className="h-12 w-12 overflow-hidden rounded-full" /></div><p className="font-semibold">Meet your SturdiHome guides.</p><p className="mt-1"><strong>Sturdi Girl is your guide.</strong> She helps you navigate services, financing options, vendors, and appointments. Brixy is her wise, funny companion who keeps the conversation grounded.</p><p className="mt-1">Choose either character above, type a question, and they will help with the next step.</p><button type="button" onClick={dismissIntro} className="mt-2 font-semibold underline">Got it</button></div>}
    ...
    <button type="button" onClick={() => setOpen((value) => !value)} aria-label={open ? "Close character chat" : "Ask Sturdi Girl"} className="flex items-center gap-2 rounded-full bg-brand-navy px-3 py-2.5 text-sm font-semibold text-white shadow-lg hover:bg-brand-navy/90 sm:px-4 sm:py-3">
      {open ? "Close" : <><span className="flex -space-x-2" aria-hidden="true"><CharacterStage character="sturdiGirl" className="h-8 w-8 overflow-hidden rounded-full border-2 border-brand-gold sm:h-9 sm:w-9" /><CharacterStage character="brixy" className="h-8 w-8 overflow-hidden rounded-full border-2 border-brand-gold sm:h-9 sm:w-9" /></span><span>Ask Sturdi Girl</span></>}
    </button>
  </div>;
}
