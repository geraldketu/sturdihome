...
type BixySettings = { enabled: boolean; showOnPages: string[]; maintenanceMode: boolean; maintenanceMessage: string; greeting: string; voiceOutputEnabled: boolean; voiceInputEnabled: boolean; typedChatEnabled: boolean; animationMode: string; mouthAnimationEnabled: boolean; guidanceEnabled: boolean; pageHelpEnabled: boolean; sessionPersistence: boolean; freePreviewSeconds: number; paidAccessEnabled: boolean; pricingJson: readonly { id: string; label: string; amountCents: number; enabled: boolean }[] };
type RecognitionLike = { continuous: boolean; interimResults: boolean; lang: string; start: () => void; stop: () => void; onresult: ((event: { results: { [index: number]: { [index: number]: { transcript: string } } } }) => void) | null; onerror: (() => void) | null; onend: (() => void) | null };
...
export default function ChatWidget({ authenticated = false, initialStatus = null, paymentsEnabled = false, bixySettings }: { authenticated?: boolean; initialStatus?: Status | null; paymentsEnabled?: boolean; bixySettings: BixySettings }) {
  const pathname = usePathname() ?? "/";
  const pageEnabled = bixySettings.showOnPages.includes("*") || bixySettings.showOnPages.some(page => pathname === page || pathname.startsWith(`${page}/`));
  ...
  const { messages, sendMessage, status: chatStatus, error } = useChat<UIMessage>({ transport: new DefaultChatTransport({ api: "/api/chat" }), messages: [{ id: "greeting", role: "assistant", parts: [{ type: "text", text: bixySettings.greeting || GREETINGS.brixy }] }] });
  ...
  if (!bixySettings.enabled || !pageEnabled) return null;

  function dismissIntro() { window.localStorage.setItem("sturdihome-character-intro-seen", "1"); setIntro(false); }
  function handleSubmit(event: React.FormEvent) { event.preventDefault(); if (!authenticated || busy || !bixySettings.typedChatEnabled || !input.trim()) return; sendMessage({ text: input.trim() }, { body: { character, currentPath: pathname } }); setInput(""); }
  function toggleVoiceInput() {
    if (listening) { recognitionRef.current?.stop(); setListening(false); return; }
    const SpeechRecognition = (window as Window & { SpeechRecognition?: new () => RecognitionLike; webkitSpeechRecognition?: new () => RecognitionLike }).SpeechRecognition ?? (window as Window & { webkitSpeechRecognition?: new () => RecognitionLike }).webkitSpeechRecognition;
    if (!SpeechRecognition) return;
    const recognition = new SpeechRecognition(); recognition.continuous = false; recognition.interimResults = false; recognition.lang = "en-US"; recognition.onresult = event => { setInput(event.results[0][0].transcript); setListening(false); }; recognition.onerror = () => setListening(false); recognition.onend = () => setListening(false); recognitionRef.current = recognition; setListening(true); recognition.start();
  }
  ...
  return <div className="character-widget fixed bottom-4 right-4 z-50 flex flex-col items-end gap-3 sm:bottom-6 sm:right-6">
    {open && <div className={`character-panel character-panel-${panelMode} flex flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-xl`}>
      ...
      <CharacterInteractionPanel activeCharacter={activeCharacter} motion={speaking ? "speaking" : busy ? "listening" : "idle"} gesture={speaking ? "point-right" : "idle"} onSelect={interactWith} />
      ...
      {!busy && speechText.trim() && <ChatSpeech key={`${latestReply?.id}-${character}-${activeCharacter ?? "idle"}`} text={speechText} character={character} enabled={Boolean(activeCharacter) && messages.length > 1 && bixySettings.voiceOutputEnabled} resetToken={speechResetToken} onSpeakingChange={setSpeaking} />}
      {bixySettings.maintenanceMode ? <div className="border-t border-gray-200 p-4 text-sm text-gray-600">{bixySettings.maintenanceMessage}</div> : authenticated && status && !status.hasAccess ? <PurchaseOptions enabled={paymentsEnabled && bixySettings.paidAccessEnabled} /> : !authenticated ? <div className="space-y-2 border-t border-gray-200 p-4 text-sm"><p className="font-semibold text-brand-navy">Join SturdiHome to Continue</p><Link href="/join-network" className="block rounded-md bg-brand-navy px-4 py-3 text-center font-semibold text-white">Join SturdiHome</Link><Link href="/login" className="block text-center text-brand underline">Already a Member? Sign In</Link></div> : bixySettings.typedChatEnabled ? <form onSubmit={handleSubmit} className="flex items-center gap-2 border-t border-gray-200 p-2">{bixySettings.voiceInputEnabled && <button type="button" onClick={toggleVoiceInput} aria-label={listening ? "Stop voice input" : "Start voice input"} className={`rounded-md border px-2 py-2 text-xs ${listening ? "border-red-300 text-red-700" : "border-gray-300 text-gray-700"}`}>{listening ? "Stop" : "Mic"}</button>}<input type="text" value={input} onChange={(event) => setInput(event.target.value)} placeholder="Ask your guide..." maxLength={2000} className="min-w-0 flex-1 rounded-md border border-gray-300 px-3 py-2 text-sm text-gray-900" /><button type="submit" disabled={busy || !input.trim()} className="rounded-md bg-brand-navy px-3 py-2 text-sm font-medium text-white disabled:opacity-50">Send</button></form> : null}
    </div>}
    <button type="button" onClick={() => setOpen((value) => !value)} aria-label={open ? "Close Bixy AI" : "Talk to Bixy"} className="flex items-center gap-2 rounded-full bg-brand-navy px-3 py-2.5 text-sm font-semibold text-white shadow-lg hover:bg-brand-navy/90 sm:px-4 sm:py-3">
      {open ? "Close" : <><span className="flex -space-x-2" aria-hidden="true"><CharacterStage character="brixy" className="h-8 w-8 overflow-hidden rounded-full border-2 border-brand-gold sm:h-9 sm:w-9" /></span><span>Talk to Bixy</span></>}
    </button>
  </div>;
}
