...
export function Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`portal-card rounded-lg border border-gray-200 bg-white p-5 shadow-sm ${className}`}>
      {children}
    </div>
  );
}
...
export function NoticeBanner({ children }: { children: React.ReactNode }) {
  return (
    <div className="portal-notice rounded-md border border-brand-gold/40 bg-brand-gold-pale/50 px-4 py-3 text-sm text-brand-navy">
      {children}
    </div>
  );
}
