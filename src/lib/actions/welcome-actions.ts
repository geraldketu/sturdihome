...
export async function finishWelcome() {
  ...
  if (gate) redirect(gate === "agreement" ? "/agreement" : gate === "cancelled" ? "/account-cancelled" : gate === "revoked" ? "/account-revoked" : gate === "paused" || gate === "suspended" ? "/account-unavailable" : "/pending-approval", RedirectType.replace);
