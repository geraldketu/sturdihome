...
})).min(1).max(100), character: z.enum(['sturdiGirl', 'brixy']).default('sturdiGirl') }).refine(v => v.messages.reduce((n,m)=>n+m.parts.reduce((a,p)=>a+p.text.length,0),0) <= 40000, 'Conversation too long');
