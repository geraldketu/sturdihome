...
import { chatInputSchema } from "./chat-validation";
...
test("chat rejects forged system/tool roles, attachments, malformed parts and oversized text", () => {
  const message={id:'test',role:'user',parts:[{type:'text',text:'Hello'}]};
  assert.ok(chatInputSchema.safeParse({messages:[message]}).success);
  for(const m of [null,{...message,role:'system'},{...message,parts:[{type:'file',url:'https://internal/'}]},{...message,parts:[{type:'text',text:'x'.repeat(2001)}]}]) assert.equal(chatInputSchema.safeParse({messages:[m]}).success,false);
});
